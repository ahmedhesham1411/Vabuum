...
public class Cart extends AppCompatActivity implements CartAdapter.OnClickHander {
    FirebaseUser fuser;
    DatabaseReference reference;
    AlertDialog alertDialog;
    Context context;
    String user_id;
    List<ItemCart> listitems;
    CartAdapter subCatAdapter;
    RecyclerView items_recycler;
    MyButtonBold yes,no,done;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        context = this;
        items_recycler = findViewById(R.id.mycart);
    ...
    private void getData() {
        ...
        reference = FirebaseDatabase.getInstance().getReference("My Cart").child(user_id);
        reference.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                listitems.clear();
                for (DataSnapshot Snapshot : dataSnapshot.getChildren()){
                    ItemCart item = Snapshot.getValue(ItemCart.class);
                    listitems.add(item);
                    items_recycler.setLayoutManager(new LinearLayoutManager(context));
                    items_recycler.setHasFixedSize(true);

                    subCatAdapter = new CartAdapter(listitems,getApplicationContext(), Cart.this);
